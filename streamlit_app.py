...
def calcular_score(row):
    """
    Score compuesto para rankear apuestas.
    Combina edge, cuota óptima y penaliza empates.
    """
    score = row['edge']

    # Bonus por cuota en zona óptima (1.70 - 3.50)
    cuota = row['cuota']
    if 1.70 <= cuota <= 3.50:
        score += 3.0
    ...
    # Penalización leve por empate (tipo D) — menos predecible
    if row['tipo'] == 'D':
        score -= 2.0

    # Bonus leve por edge muy alto
    if row['edge'] >= 10:
        score += 2.0
    elif row['edge'] >= 7:
        score += 1.0
...
def generar_ranking(df, top_n=15, edge_minimo=5.0, excluir_empates=False):
    """
    Filtra y rankea las mejores apuestas del reporte.
    """
    df = df.copy()
    ...
    # Filtro opcional de empates
    if excluir_empates:
        df = df[df['tipo'] != 'D']

    # Cuota en rango razonable
...
def mostrar_ranking(df_rank):
    """Muestra las apuestas como tarjetas visuales."""
    ...
    for i, row in df_rank.iterrows():
        ...
        with col2:
            st.markdown(f"""
                <div class='rank-liga'>{row['liga']}</div>
...
with tab1:
    ...
    if os.path.exists(csv_path):
        ...
        if df_rank.empty:
            ...
            # Aviso sobre empates si aparecen
            ...
            csv_export = df_rank.drop(columns=['score']).to_csv(index=True).encode('utf-8')
            st.download_button(
                label="Descargar como CSV",
                data=csv_export,
                file_name=f"ranking_value_bets.csv",
                mime="text/csv"
            )

    else:
        st.info("Todavía no hay reporte generado. Ejecutá el backtesting o la detección en vivo primero.")
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown("""
        **Cómo empezar:**
        1. Ir a la pestaña **📊 Backtesting Histórico**
        2. Hacer click en **Ejecutar backtesting**
        3. Volver acá para ver el ranking
        """)
...
with tab2:
    ...
    if st.button("▶️ Ejecutar backtesting", key="btn_backtest"):
        try:
            ...
            if apuestas:
                st.success(f"✅ Completado — {len(apuestas)} apuestas analizadas en {len(resumen)} ligas")

                st.subheader("Resumen por liga")
                df_resumen = pd.DataFrame(resumen)

                # Colorear yield positivo/negativo
                def color_yield(val):
                ...
                st.subheader("Detalle completo de apuestas")
...
with tab3:
    ...
    st.markdown("Busca partidos sin resultado todavía y detecta oportunidades según el modelo.")

    if st.button("▶️ Ejecutar análisis de mercado", key="btn_live"):
        try:
            import main
            with st.spinner("Analizando mercados en vivo..."):
                apuestas = main.run()
            ...
            if apuestas:
                ...
                st.info("No se detectaron value bets en esta jornada. Puede que no haya partidos próximos con suficientes datos.")
