...
with st.sidebar:
    ...
    # API Key -- lectura robusta para Streamlit Cloud
    api_key_input = ""
    api_key_fuente = ""
    ...
    try:
        if "ODDS_API_KEY" in st.secrets:
            api_key_input = str(st.secrets["ODDS_API_KEY"]).strip()
            api_key_fuente = "secrets"
    except Exception:
        pass
    ...
    if not api_key_input:
        import os
        api_key_input = os.environ.get("ODDS_API_KEY", "").strip()
        if api_key_input:
            api_key_fuente = "env"

    # 3) Siempre mostrar el campo -- permite override manual
    api_key_manual = st.text_input(
        "🔑 The Odds API Key",
        value=api_key_input if api_key_input else "",
        type="password",
        placeholder="Pega tu key de the-odds-api.com",
        help="Configurala en Streamlit Cloud > Settings > Secrets como ODDS_API_KEY = 'tu-key'"
    )
    # El campo manual tiene prioridad si el usuario escribe algo
    if api_key_manual:
        api_key_input = api_key_manual.strip()

    # Indicador visual del estado
    if api_key_input:
        src_label = f" ({api_key_fuente})" if api_key_fuente else ""
        st.caption(f'✅ API key OK {src_label}')
    else:
        st.caption('⚠️ Sin API key — verás HTTP 401')
        st.caption('Ir a Settings > Secrets: ODDS_API_KEY = tu-key')
    ...
    st.markdown("---")
    scan_live = st.button("🔍 ESCANEAR MERCADOS", width='stretch')
    scan_back = st.button("📊 BACKTESTING", width='stretch')
...
with tab_back:
    ...
    if scan_back or st.session_state.get("bt_data") is not None:
        ...
        if resumen:
            ...
            st.dataframe(
                df_res.style.map(color_yield, subset=["Yield %", "P&L u"]),
                width='stretch',
                hide_index=True
            )
        ...
        if registros:
            ...
            st.dataframe(
                df_det_fil.style.map(color_res, subset=["Res"]),
                width='stretch',
                hide_index=True
            )
